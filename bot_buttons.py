...
@dp.callback_query(lambda callback: callback.data == "random_value")
async def send_random_value(callback: types.CallbackQuery):
    user = callback.from_user
    random_number = randint(1, 10)

    # Ответить пользователю, который нажал на кнопку
    await callback.message.answer(
        f"Пользователь {user.username} ({user.id}) выбрал число: {random_number}"
    )

    # Уведомить пользователя о получении callback
    await callback.answer(
        text=f"Спасибо за выбор, {user.username}! Ваше число: {random_number}",
        show_alert=True
    )
